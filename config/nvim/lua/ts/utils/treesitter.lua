...
function M.build(cb)
	M.ensure_treesitter_cli(function(_, err)
		...
		if ok then
			...
			notify.error(lines, { title = "LazyVim Treesitter" })
		end
	end)
end
...
function M.ensure_treesitter_cli(cb)
	...
	mr.refresh(function()
		...
		if not p:is_installed() then
			notify.info("Installing `tree-sitter-cli` with `mason.nvim`...")
			p:install(
				nil,
				vim.schedule_wrap(function(success)
					if success then
						notify.info("Installed `tree-sitter-cli` with `mason.nvim`.")
						cb(true)
					else
